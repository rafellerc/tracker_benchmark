import math
# from scripts import *
import scripts.butil.calc_rect_center as calc_center
from scripts.butil import extra
...
def calc_seq_err_robust(results, rect_anno):
    ...
    if resultType == 'rect':
        ...
        for i in range(seq_length):
            ...
            rect = calc_center.rect_affine_IVT(results.tmplsize, res[i])
            rectMat[i] = rect
    elif resultType == 'L1Aff' or resultType == 'affine_L1':
        for i in range(seq_length):
            # rect, c = scripts.butil.calc_center_L1(res[i], results['tmplsize'])
            rect = calc_center.rect_affine_L1(results.tmplsize, res[i])
            rectMat[i] = rect
    elif resultType == 'LK_Aff' or resultType == 'affine_LK':
        for i in range(seq_length):
            rect = calc_center.rect_affine_LK(results.tmplsize, 
                                              res[2*i:2*(i+1)])
            rectMat[i] = rect
        ...
        for i in range(seq_length):
            ...
            rect = calc_center.rect_4corners(res[2*i:2*(i+1)])
            rectMat[i] = rect
        ...
        for i in range(seq_length):
            rect = calc_center.rect_similarity(results.tmplsize, res[i])
            rectMat[i] = rect
    ...
    errCenter = [round(extra.ssd(center[i], centerGT[i]), 4)
                 for i in range(seq_length)]
