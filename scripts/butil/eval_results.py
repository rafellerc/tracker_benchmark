import config
# from scripts import *
import scripts.butil.calc_seq_err_robust as calc_robust
from scripts.model.score import Score, getScoreList
...
def calc_result(tracker, seqs, results, evalType):
    ...
    for i in range(len(results)):
        ...
        seq = next(seq for seq in seqs if seq.name.lower() ==
                   subResults[0].seqName.lower())
        seq.aveCoverage = []
        seq.aveErrCenter = []
        seq.errCoverage = []
        seq.errCenter = []
        ...
        for j in range(idxNum):
            ...
            aveCoverage, aveErrCenter, errCoverage, errCenter = \
                calc_robust.calc_seq_err_robust(result, anno)
    ...
    for attr in attrList:
        ...
        for seq in seqs:
            if attr.name in seq.attributes or attr.name.lower() == 'all':
                ...
                for threshold in config.thresholdSetOverlap:
        ...
        if len(successRateList) > 0:
            for i in range(len(config.thresholdSetOverlap)):
                attr.successRateList.append(
                    sum([rates[i] for rates in successRateList]) /
                    float(len(successRateList)))
        attr.refresh_dict()
    # end for scores
